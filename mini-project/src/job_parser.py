...
class JobParser:
    ...
    def parse_job(self, job: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract information
        # Handle both 'required_skills' and 'requirements' columns
        required_skills = job.get("required_skills", [])
        if not required_skills and "requirements" in job:
            # Parse requirements string into list
            requirements_str = job["requirements"]
            if isinstance(requirements_str, str):
                required_skills = [skill.strip() for skill in requirements_str.split(",")]

        parsed_job["required_skills"] = required_skills
        parsed_job["extracted_skills"] = self._extract_skills(description, required_skills)
