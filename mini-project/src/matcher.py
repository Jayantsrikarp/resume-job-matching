...
class Matcher:
    ...
    def _calculate_skill_match_score(self, resume: Dict[str, Any], job: Dict[str, Any]) -> float:
        """Calculate skill match score between resume and job using partial matching.
        ...
        if "required_skills" in job and job["required_skills"]:
            if isinstance(job["required_skills"], list):
                job_skills.extend([skill.lower() for skill in job["required_skills"]])
            elif isinstance(job["required_skills"], str):
                job_skills.append(job["required_skills"].lower())

        # Then try extracted_skills
        if "extracted_skills" in job and job["extracted_skills"]:
            job_skills.extend([skill.lower() for skill in job["extracted_skills"]])

        # If no skills found, return 0
        if not resume_skills or not job_skills:
            return 0.0

        # Find matching skills using partial matching
        matching_skills = set()
        ...
        # Calculate Jaccard similarity with all unique skills
        all_skills = set(resume_skills).union(set(job_skills))
    ...
    def _get_matching_skills(self, resume: Dict[str, Any], job: Dict[str, Any]) -> List[str]:
        """Get list of skills that match between resume and job using partial matching.
        ...
        """
        # Get resume skills with original case mapping
        resume_skills = {}
        if "skills" in resume and resume["skills"]:
            resume_skills = {skill.lower(): skill for skill in resume["skills"]}

        # Get job skills with original case mapping
        job_skills = {}
        ...
        # Find matching skills using partial matching
        matching_skills = []
        for job_skill_lower, job_skill_original in job_skills.items():
            for resume_skill_lower, resume_skill_original in resume_skills.items():
                # Check for exact match or partial match (either direction)
                if (job_skill_lower == resume_skill_lower or
                    job_skill_lower in resume_skill_lower or
                    resume_skill_lower in job_skill_lower):
                    matching_skills.append(job_skill_original)  # Use original case from job
                    break

        return matching_skills
    
    def _get_missing_skills(self, resume: Dict[str, Any], job: Dict[str, Any]) -> List[str]:
        """Get list of skills required by the job but missing from the resume using partial matching.
        ...
        # Find missing skills using partial matching
        missing_skills = []
        for job_skill_lower, job_skill_original in job_skills.items():
            is_matched = False
            for resume_skill_lower in resume_skills:
                # Check for exact match or partial match (either direction)
                if (job_skill_lower == resume_skill_lower or
                    job_skill_lower in resume_skill_lower or
                    resume_skill_lower in job_skill_lower):
            ...
            if not is_matched:
                missing_skills.append(job_skill_original)  # Use original case from job
