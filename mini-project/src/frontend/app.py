...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # Run main.py with the uploaded resume
        try:
            import subprocess
            main_script_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'main.py')
            jobs_path = "C:\\Users\\ASUS\\Downloads\\processed_job_data.csv"
            # D:\\tmp\\mini-project\\job_market_data.csv  # Path to job listings file
            output_dir = app.config['OUTPUT_FOLDER']
            
            # Run the main.py script with the uploaded resume
            cmd = [
                sys.executable,
                main_script_path,
                '--resume', file_path,
            ...
            if process.returncode != 0:
                logger.error(f"Error running main.py: {process.stderr}")
                flash('Error processing resume')
                return redirect(url_for('index'))
            
            # Get the report filename from the output directory
            # The report filename format is report_{resume_name}.html
            resume_name = os.path.splitext(os.path.basename(file_path))[0]
            report_files = [f for f in os.listdir(output_dir) if f.startswith('report_') and f.endswith('.html')]
            
            # Sort by creation time to get the most recent report
            report_files.sort(key=lambda x: os.path.getctime(os.path.join(output_dir, x)), reverse=True)
